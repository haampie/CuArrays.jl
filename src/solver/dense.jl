using Base.LinAlg: BlasInt, checksquare
using Base.LinAlg.LAPACK: chkargsok
...
for (bname, fname,elty) in ((:cusolverDnSpotrf_bufferSize, :cusolverDnSpotrf, :Float32),
    ...
    @eval begin
        function Base.LinAlg.LAPACK.potrf!(uplo::BlasChar,
                        A::CuMatrix{$elty})
            ...
            chkargsok(BlasInt(info)) # BlasInt restriction of chkargsok should be loosened
            A, info
        end
    end
end
...
for (fname,elty) in ((:cusolverDnSpotrs, :Float32),
    ...
    @eval begin
        function Base.LinAlg.LAPACK.potrs!(uplo::BlasChar,
                        A::CuMatrix{$elty},
                        B::CuVecOrMat{$elty})
            cuuplo = cublasfill(uplo)
            n = checksquare(A)
